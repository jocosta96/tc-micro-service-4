...
# Get security group ID from SSM - handle errors internally
...
if aws ec2 describe-security-group-rules \
  --region "${REGION}" \
  --filters "Name=group-id,Values=${SG_ID}" "Name=cidr,Values=${CIDR}" "Name=from-port,Values=443" \
...
# Add rule - handle errors internally
